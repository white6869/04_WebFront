...
// number1, number2 합 출력하가ㅣ
console.log(number1 + number2);

// 변수에 대입한 값 변경하기
number1 = 300; // number1에 300 대입
number2 = 400; // number2에 400 대입
...
// 1. var (변수, 변수 선언 시 중복되는 변수명으로 선언가능)
...
// 이전에 저장 해놓은 변수값을 쓸 수 없게 되는 문제가 발생함.
...
// -> 상수에 새로운값을 대입할 수 없는데, 재대입을 하고있어 오류사항이다.
...
// 블록 레벨 sceope (let, const)
// let const로 선언된 변수는
// 변수가 선언된 블록 ( 중괄호 {} ) 내부에서만 유효하다는 것을 의미함.
// 즉, 코듭블록 내부에서 선언한 변수(let, const)는 해당 블록 외부에서는 접근할 수 없다.
...
// var 키워드로 선언된 변수는
//  함수내 어디서는 접근할 수 있음.
// if, for 등의 일반블록은 무시하고, var로 선언된 변수가 있는 함수 전체에서 유효함.
...
console.log(test2);
// -> test2도 출력 가능.
// -> 왜? 함수레벨 스크프인 var는 일반블록은 무시하기 때문인다.
...
// -> var는 일반블록레벨만 무시할 뿐 함수레벨 블록은 무시하지 못함
...
// 1. var의 호이스팅 : var로 선언된 변수는 선언만 해당 스코프의 최상단으로
...
// 2. let / const 의 호이스팅 : let/const로 서언된 변수는 선언만 해당 스코프의 최상단으로 호이스팅 됨.
// var와 달리, let/const로 선언된 벼수는 초기화가 이루어지기 전에 사용X
...
console.log("userName : " , phone, typeof phone);
